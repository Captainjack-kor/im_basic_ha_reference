...
describe('App test', () => {
  ...
  test('영화 목록을 클릭할 때에, `handleCardClick` 메서드를 이용해서 현재 영화정보를 업데이트해야 합니다.', async () => {
    container = document.createElement('div');
    document.body.appendChild(container);
    ...
    await act(async () => {
      App.prototype.handleCardClick = jest.fn();
      ReactDOM.render(<App />, container);
    });
    ...
    expect(App.prototype.handleCardClick).toBeCalledTimes(1);
